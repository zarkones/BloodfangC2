 ...
 */
auto declfn resolve::module(_In_ const uint32_t library_hash) -> uintptr_t {
  //
  ...
  RangeHeadList(NtCurrentPeb()->Ldr->InLoadOrderModuleList,
                PLDR_DATA_TABLE_ENTRY,
                {
                  ...
                  if (bloodfang::hash_string<wchar_t>(
                          Entry->BaseDllName.Buffer) == library_hash) {
                    return reinterpret_cast<uintptr_t>(Entry->DllBase);
                  }
                })
 ...
 */
auto declfn resolve::_api(_In_ const uintptr_t module_base,
                          _In_ const uintptr_t symbol_hash) -> uintptr_t {
  ...
  nt_header =
      reinterpret_cast<PIMAGE_NT_HEADERS>(module_base + dos_header->e_lfanew);
  ...
  export_dir = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(
      ...
      nt_header->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]
          .VirtualAddress);
  export_names =
      reinterpret_cast<PDWORD>(module_base + export_dir->AddressOfNames);
  export_addrs =
      reinterpret_cast<PDWORD>(module_base + export_dir->AddressOfFunctions);
  export_ordns =
      reinterpret_cast<PWORD>(module_base + export_dir->AddressOfNameOrdinals);

  for (int i = 0; i < export_dir->NumberOfNames; i++) {
    symbol_name = reinterpret_cast<PSTR>(module_base + export_names[i]);

    if (bloodfang::hash_string(symbol_name) != symbol_hash) {
      continue;
    }
